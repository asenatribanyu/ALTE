    ...
    <span
        ...
        @auth
            @if (auth()->user()->document()->latest()->first())
                @if (auth()->user()->document()->latest()->first()->created_at->diffInDays(now()) > 7 &&
                        auth()->user()->document()->latest()->first()->tipe == $formulir->name)
                    PDF
                @elseif(auth()->user()->document()->latest()->first()->created_at->diffInDays(now()) <= 7 &&
                        auth()->user()->document()->latest()->first()->tipe == $formulir->name)
